...
def color_plausibility(img_path):
    # Load image as RGB float in [0, 1]
    ...
    # Calculate mean and std of a* and b* channels
    a_mean, b_mean = np.mean(lab[:, :, 1]), np.mean(lab[:, :, 2])
    a_std, b_std   = np.std(lab[:, :, 1]), np.std(lab[:, :, 2])
    
    # Compute deviation and chroma variance
...
import cv2
import torch
from basicsr.metrics import calculate_niqe  # or wherever your NIQE function is

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
def compute_no_ref_metrics(img):
    """
    img: numpy array in grayscale (H, W) or RGB/BGR
    Calculates NIQE on grayscale version.
    """
    # If already grayscale, keep it
    if img.ndim == 2:  
        gray = img
    else:
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # or COLOR_RGB2GRAY if needed

    # NIQE expects HWC format
    gray_3ch = cv2.cvtColor(gray, cv2.COLOR_GRAY2RGB)

    # Convert to tensor (not actually needed for NIQE, but if you want consistency)
    img_tensor = torch.tensor(gray_3ch.transpose(2, 0, 1)).unsqueeze(0).float() / 255.0
    img_tensor = img_tensor.to(device)

    niqe_score = calculate_niqe(gray_3ch, crop_border=0, input_order='HWC', convert_to='y')
    return float(niqe_score)
def start(img_path):
    cp = color_plausibility(img_path) 
    print("cp:", cp)

    image = cv2.imread(img_path)  # color read
    niqe_score = compute_no_ref_metrics(image)  # will convert to gray inside
    print('niqescore:', niqe_score)

    print("final score", 0.5 * niqe_score + 0.5 * cp)

start('/content/image-5.jpg')
